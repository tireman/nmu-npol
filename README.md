...
1.  Run the cleanBuild script.  It will attempt to build the code from scratch.
	This new version builds an "analysis" binary, a "npolib" class, and
	a "simulation" binary which can be found in the ``build/analysis", 
	"build/npollib", or "build/simulation`` directories. 
  ```
    % ./cleanBuild
    % cd build/analysis
	% cd build/npollib
...
1.  Create a three directories inside nmu-npol/ called build/analysis, build/npollib, and build/simulation
...
### Environmental variable and directory setup scripts for simulation and analysis
	Shell scripts to setup the necessary environmental variables are contained in the
	directory labeled "env_setup" which resides in the base directory ("nmu-npol").
	There are three primary scripts which setup the simulation run and analysis ENV
	variables.

	1) ClusterAnalysis.sh <-- This file is designed for running on the NMU Math & CS
	cluster.  Currently (March 2019) it is not usuable due to internal problems.
	This may become defunct.

	2) JLABsetupAnalysis.csh <-- This file is designed for running on the JLAB
	farm compute nodes.  It is specialized for the analysis code.

	3) NMUnpolVariables.sh <-- This file is the newly minted shell script for setting
	all ENV variables on the NMU Physics Department machines.  This works for running
	the simulation and the analysis code.
...
### Macro files
	...
	This macro projects a beam of electrons with circular profile of 2 mm in the
	...
	polarimeter.  This is usually ran with all target and beam line elements turned off.

	3) Biased particle flux generated at a "tagger" volume
	File name: ParticleFlux.mac (plus several other versions)

	Update: In the nmu-npol/simulation/macros directory, there are directories Run4.4GeV and Run11GeV
	...
	very useful when working on a single multi-core machine such as the 28 core (56 thread) 
	...
	Hall Shell, scattering chamber.	If generating at NPOL tagger then comment out shield hut 
	...
	application requires 5 biasing histograms AT the tagger volume in question. The x-y distributions must
	match the size of the distributions in the real simulation!  These histograms
	...
	1) The first way is to placed a 40-cm LD2 target in the scattering chamber and 3 tagger volumes 
	...
	Tagger 3: "SHMS tagger" is a thin volume placed very close to the horizontal bender's entrance.
	...
	2) The second way to generate the histograms for the biasing technique is to use another simulation
	such as SimC to generate the particle flux from the QE electron-deuteron scattering.  This was done
	in fall 2017 in a very basic run using LH2 and then calling protons neutrons (I know).  
	It takes a ROOT script to convert from the SimC coordinate system to the Npolapp setup, however, 
	it can and has been done.  A future description of this may be written but right now there are 
	other issues to work on (12-March-2019).
...
1.  You can run in batch mode on either the Jlab Farm or on a local machine. Currently there are 
	...
	simluation/scripts/NMUPhysics directory. You could also modify the NMUnpolVariables.sh script to match 
	your plans, however, the simulation will default to a local output directory and filename if this 
	is not done.
	
	Then run the simualtion by: 
  ```
    % cd build/simulation							# if not already in this directory
    % source ../../env_setup/NMUnpolVariables.sh   	# sets up output dir and name (optional); 
      	     			     		  				# change as needed
    % ./Npolapp macros/ElectronBeam.mac				# choose your macro file from above or create your own
  ```
	If you want to run a set of runs on a muti-core machine, use the script NMUjobsubmit.sh. or one 
...
3.	On ALL systems, you can run interactive sessions with a macro file by running 
	...
	ENV setup file was not ran before this command, then the output will be stored 
	locally in build/simulation/output.  The macro files can also be stored in
	subdirectories of macro/. 

4.  The "data" are outputted in a root file as per NpolFileManager.  You will
    have to change the directory to which the data file(s) are written via the 
	ENV variables in the NMUnpolVariables.sh or JLABsetuprun.csh script.  The default 
