...
int getSectionOfInterest(const std::map<std::string,NpolDetectorEvent *> *detEvents) {
	...
	for(int section = 3; section >= 0; section--) {
		...
		if((analyzerFlag && topEArrayFlag && topdEArrayFlag) || (analyzerFlag && botEArrayFlag && botdEArrayFlag)) sectionOfInterest = section; // If this section passes requirement 1, then it may be the section of interest
	}
...
*/
		...
		if(sectionOfInterest != -1) {
			...
			else dEArrayOfInterest = botdEArray;
	...
	h_sectionEfficiency1->Scale(1.0/totalEvents);
	h_sectionEfficiency2->Scale(1.0/totalEvents);
	h_sectionEfficiency3->Scale(1.0/totalEvents);
	h_sectionEfficiency4->Scale(1.0/totalEvents);
