...
void EstimateCrossSection() {
 ...
 void FillCanvas(TCanvas *C, Double_t scaleFactor, TFile *inFile, std::string histoNames[3][3], struct histoParams plotSettings){
  ...
  Double_t molarMass = 2.014;
  Double_t AvogradoNumber = 6.0222e23;
  Double_t thickness = 40.0;
  Double_t densityD = 0.169;
  ...
  std::cout << "Total Number of " << histoNames[i][j] << ": " << totalCounts << " Counts" << std::endl;
 
  std::cout << "Cross Section for " << histoNames[i][j]  << ":" << totalCounts/(TotalElectrons*solidAngle*nucleiPerVolume*thickness)/1e-24/1e-6 << " microbarns/sr\n" << endl; 
...
TString FormOutputFile(TString OutputDir){
  ...
  TString fileName =  OutputDir + "/" + BaseName + Energy + "GeV_Lead" + Lead + "cm_" + Bfield + "Bdl_CrossSection.root";
