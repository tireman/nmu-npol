...
G4AssemblyVolume *NpolPolarimeter::MakePlate(G4LogicalVolume *detLV, G4int numDets, 
...
void NpolPolarimeter::ImprintPlate(G4AssemblyVolume *plate, G4LogicalVolume *motherLV,
...
=======
	...
	G4VisAttributes* FrontDetVisAtt= new G4VisAttributes(G4Colour(1.0,1.0,0.0));
	FrontDetLV->SetVisAttributes(FrontDetVisAtt);
>>>>>>> b2931d54c709ec67ebdce6e45941993c95ee2698
}
...
=======
	...
	for(unsigned int i=0; i<2; i++)
		ImprintPlate(FrontTaggerArray1, motherLV, 0.0*m, 0.0*m, (-1.7692+0.65*i)*m, 0.0*deg);
	for(unsigned int i=2; i<4; i++)
		ImprintPlate(FrontTaggerArray2, motherLV, 0.0*m, 0.0*m, (-1.7692+0.65*i)*m, 0.0*deg);
>>>>>>> b2931d54c709ec67ebdce6e45941993c95ee2698
}
...
G4VPhysicalVolume *NpolPolarimeter::Construct(G4LogicalVolume *motherLV) {
	...
	return PlaceCylindrical(PolarimeterLV,motherLV,"Polarimeter",8.7*m,26.0*deg,0.0*m);
}
